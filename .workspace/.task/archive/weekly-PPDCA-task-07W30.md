...
## 2026-09-06 棚卸し（archive移動時の判定）

`.workspace/.task/weekly-PPDCA-task-07W30.md` 内「解析中に浮かんだタスク（W31以降）」の各項目を、7〜9月に実施済みの大型施策（`/en/`廃止・67件リダイレクト・Tier A〜D構造強化・本音と建前リライトTier1〜4）と現行マスターファイルに照らして判定した。

- <strong>生存（`weekly-task.md`へ転記済み・重複のため本ファイルは不要）</strong>: 音大生シェアハウス地域別クエリ（line 111）／大阪・関西防音賃貸クエリ（line 132、なお`osaka-soundproof-rental-guide`本文は2026-09-06のTier B施策で拡充済み）／Bing流入が主軸（line 129）
- <strong>新規タスク化</strong>: 「防音ラボ」指名検索は認知度不足の課題として`task-list.md`「『防音Lab』ブランド認知向上ロードマップ」に追加
- <strong>陳腐化・対応済みで不要</strong>:
  - `/ja`トップのエンゲージメント率 → persona-hub再配置は実施済み、以降はGA4データ途絶（下記）で単独追跡不能
  - 順位40位台の高表示ページ2本のうち`absorption-vs-soundproofing-materials` → Tier A内部リンク追加で対応済み（2026-09-06実データで順位50.4→22.4改善を確認）
  - ベーストラップ表記ゆれ → `bass-trap-installation-guide`は既にサイト最大の成功記事。以降どのマスターにも引き継がれず優先度から自然に外れたと判断
  - GA4デバイスセグメント要望 → 最新`W36`フォルダはGSCのみでGA4エクスポート自体が存在せず、前提が崩壊。データ取得フロー自体の確認が先
- <strong>2026-09-06訂正: 「不要」判定を差し戻した項目</strong>（`/weekly-ppdca`スキルの初回テスト実行でW36実データを確認した結果、判定が誤りと判明）:
  - 「東京 防音 工事 補助金」順位41.47 → 当初「Tier施策で前提が変化済み」として不要判定したが、W36実データで順位41.44とほぼ無風であることを確認。誤りだったため`task-list.md`に差し戻し
  - `construction-types-cost-comparison` → 当初「Tier施策で対応済み」としたが、W36実データで順位40.3・クリック0のまま変化なしを確認。`weekly-task.md`のTier ABCD効果測定フォローアップに観察対象として追加
