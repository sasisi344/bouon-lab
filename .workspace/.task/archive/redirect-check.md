...
1. **GSCの1000件はUI表示上限による切り詰め**（実際の合計は2,592件、インデックス作成レポートには期間フィルタなし）。サーバーのアクセスログは保存無効のため全件抽出は不可。**運用方針**: GSCの404一覧は「直近クロール日順」で並ぶため、今回対応した895件（en・tags・categories）が再クロールで解消され次第、リストから外れて次のバッチが繰り上がる。**2,592件→減少が確認できたタイミングで、同じ手順（CSVエクスポート→全件解析→astro.config.mjs追記→ビルド→デプロイ）をもう一度実施すればよい**（ユーザー合意済み、2026-09-05）。目安は再クロール完了までの1〜2ヶ月後
2. **GSC削除リクエストの完了確認**。送信直後のため「処理中」ステータス。数日後にGSCの「非表示」画面で「完了」になっているか確認するとよい
3. `.workspace/.task/movie/.skills/movie-generation-rules.md`の削除が未コミットのまま残っている（今回の作業とは無関係、別途確認・対応が必要）