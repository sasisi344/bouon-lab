...
| G4 ✅ | <strong>local 15本の統合設計</strong>: 2026-07-20完了。振り分け表を確定し、ハブ記事`bouon-rental-market-guide`と17都市ガイドを双方向リンクで接続（未接続12本にハブ→都市・都市→ハブの相互リンクを追加）。noindex追加実行は見送り（下記振り分け表参照） | <strong>2026-07-20実行予定</strong>（W30データ入手後） | 達成。振り分け表は下記「local 17本の振り分け表（G4成果物）」参照 |
...
| 中 | local 15本（表示ゼロ） | 新規凍結。bouon-rental-market-guide をハブに導線集約し、リンク薄い都市ページは統合検討 → 2026-07-20実施（下記振り分け表） |
| 中 | creator 表示ゼロ39本 | streamer-soundproof-room-comprehensive-guide への内部リンク集約とカニバリ確認 |

## local 17本の振り分け表（G4成果物・2026-07-20確定）

W30時点（過去3か月集計）でGSCに表示があるのは chiba・osaka・kyoto・saitama・sendai のみ（各1〜3表示）で、直近期間はいずれも0。実質17本中17本が需要ゼロに近い状態。個別記事の「6週連続表示ゼロ→noindex」の最終判定は6週データが単独で揃っていないため今回は見送り、まず<strong>導線集約（ハブとの双方向リンク）</strong>を実行し、次のGSC観測（W32〜W33目安）で個別noindex判定を行う。

| 都市 | 判定 | 理由 | 今回の対応 |
| :--- | :--- | :--- | :--- |
| tokyo | 残す（主要） | 首都圏最大市場、既にハブ双方向リンクあり | 変更なし |
| osaka | 残す（主要） | W30も表示あり、既にハブ双方向リンクあり | 変更なし |
| kobe | 残す（主要） | 関西主要都市、既にハブ双方向リンクあり | 変更なし |
| chiba | 残す（主要） | W30も表示あり、既にハブ双方向リンクあり | 変更なし |
| kanagawa | 残す | 首都圏・価格表に記載済みだがハブ未リンクだった | ハブ↔記事の双方向リンク追加 |
| saitama | 残す | 首都圏・価格表に記載済みだがハブ未リンクだった、W30表示あり | ハブ↔記事の双方向リンク追加 |
| nagoya | 残す | 中部主要都市・価格表に記載済みだがハブ未リンクだった | ハブ↔記事の双方向リンク追加 |
| fukuoka | 残す | 価格表に記載済みだがハブ未リンクだった | ハブ↔記事の双方向リンク追加 |
| kyoto | 残す | 音楽大学という独自切り口あり、W30表示あり | ハブ↔記事の双方向リンク追加 |
| sapporo | 残す（独自性高） | 「二重窓標準装備」という他都市にない構造的切り口 | ハブ↔記事の双方向リンク追加 |
| hiroshima | 残す（独自性高） | 「音大エリア」切り口、テンプレ量産ではない | ハブ↔記事の双方向リンク追加 |
| hamamatsu | 残す（独自性高） | 「楽器の街」切り口、テンプレ量産ではない | ハブ↔記事の双方向リンク追加 |
| kanazawa | 統合候補（要観察） | テンプレ型（駅名・大学周辺）で差別化弱い | ハブ↔記事の双方向リンク追加。W32〜W33も表示ゼロならnoindex検討 |
| okayama | 統合候補（要観察） | 同上 | 同上 |
| kumamoto | 統合候補（要観察） | 同上 | 同上 |
| niigata | 統合候補（要観察） | 同上 | 同上 |
| sendai | noindex済み | ニッチ化リライト施策の効果測定で2026-07-21にnoindex適用済み（`task-list.md`参照） | 対応不要（別トラックで完了） |

<strong>次のアクション</strong>: kanazawa・okayama・kumamoto・niigataの4本はW32〜W33のGSCで表示が引き続きゼロならnoindex化を検討する（`task-list.md`のG4後続タスクとして追記）。
